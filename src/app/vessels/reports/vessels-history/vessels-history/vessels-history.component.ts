...
import { VesselsHistory } from '../vessels-history.model';
import { VesselsHistoryService } from '../vessels-history.service';


@Component({
  selector: 'app-vessels-history',
  templateUrl: './vessels-history.component.html',
  styleUrls: ['./vessels-history.component.sass']
})
export class VesselsHistoryComponent implements OnInit {
  ...
  constructor(private fb: FormBuilder,
    ...
    public VesselsHistoryService: VesselsHistoryService,
  ...
  onDateInput2(inputValue: any, inputFlag: any, index: number) {
    ...
    if(inputValue!=""){
      if (dateFormat.test(inputValue)) {
        let fdate = this.VesselsHistoryService.getDateObj(inputValue);
        if (inputFlag == "todate") {
          this.docForm.patchValue({
            'todate': inputValue,
            'todateObj':fdate
  ...
  onDateChange2(event: any, inputFlag: any, index: number) {
    if(event.target.value!=null){
      let cdate = this.VesselsHistoryService.getDate(event.target.value);
      if (inputFlag == "todate") {

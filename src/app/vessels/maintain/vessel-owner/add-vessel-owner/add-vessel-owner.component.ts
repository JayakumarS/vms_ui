...
@Component({
  selector: 'app-add-vessel-owner',
  templateUrl: './add-vessel-owner.component.html',
  styleUrls: ['./add-vessel-owner.component.sass']
})
export class AddVesselOwnerComponent implements OnInit {
  ...
  constructor(private fb: FormBuilder,
    ...
    this.docForm = this.fb.group({
      ...
      vesselOwnerBeanDtls: this.fb.array([
        this.fb.group({
          sort : 1,
          select:[""],
   ...
   ngOnInit() {
    ...
    get rowDtls() {
      return this.docForm.get('vesselOwnerBeanDtls') as FormArray;
    }
   ...
   addRow(){
    let vesselOwnerBeanDtlsArray=this.docForm.controls.vesselOwnerBeanDtls as FormArray;
    let arraylen=vesselOwnerBeanDtlsArray.length;
    var len = this.docForm.controls["vesselOwnerBeanDtls"].value.length;
    ...
    vesselOwnerBeanDtlsArray.insert(arraylen,newUsergroup);
  }

  removeRow(){
    let count = 0;
    const deleteRow = this.docForm.controls.vesselOwnerBeanDtls as FormArray;
    let i = 0;
  ...
  fetchDetails(id){
    this.httpService.get<any>(this.VesselOwnerService.editUrl+"?id="+id).subscribe({next: (data: any) => {
      let dtlArray = this.docForm.controls.vesselOwnerBeanDtls as FormArray;
      dtlArray.clear();
  ...
  update() {
    const dtlArray = this.docForm.get('vesselOwnerBeanDtls') as FormArray;
  ...
  save(){
    if(this.docForm.valid){
      this.VesselOwnerService.saveVesselOwner(this.docForm.value, this.router, this.notificationService);
    }else{
      this.matError.markFormGroupTouched(this.docForm);
      this.notificationService.showNotification(
        "snackbar-danger",

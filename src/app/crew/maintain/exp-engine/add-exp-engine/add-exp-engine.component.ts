...
@Component({
  selector: 'app-add-exp-engine',
  templateUrl: './add-exp-engine.component.html',
...
export class AddExpEngineComponent implements OnInit {
  ...
  constructor(private fb: FormBuilder,
   ...
   ngOnInit() {
    ...
    get rowDtls() {
      return this.docForm.get('expEngineBeanBeanDtls') as FormArray;
    }
   ...
   addRow(){
    let expEngineBeanBeanDtlsArray=this.docForm.controls.expEngineBeanBeanDtls as FormArray;
    let arraylen=expEngineBeanBeanDtlsArray.length;
    var len = this.docForm.controls["expEngineBeanBeanDtls"].value.length;
    ...
    expEngineBeanBeanDtlsArray.insert(arraylen,newUsergroup);
  }

  removeRow(){
    let count = 0;
    const deleteRow = this.docForm.controls.expEngineBeanBeanDtls as FormArray;
    let i = 0;
  ...
  save(){
    if(this.docForm.valid){
      this.ExpEngineService.saveExpEngine(this.docForm.value, this.router, this.notificationService);
    }else{
      this.matError.markFormGroupTouched(this.docForm);
      this.notificationService.showNotification(
        "snackbar-danger",

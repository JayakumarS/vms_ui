...
@Component({
  ...
  styleUrls: ['./list-trainings.component.sass']
})
export class ListTrainingsComponent extends UnsubscribeOnDestroyAdapter implements OnInit {
  displayedColumns = [
   // "select",
    "code",
    "description",
  ...
  exampleDatabase: TrainingsService | null;
  selection = new SelectionModel<Trainings>(true, []);
  index: number;
  id: number;
  customerMaster: Trainings | null;
  permissionList: any=[];
  constructor(
  ...
  public loadData() {
    this.exampleDatabase = new TrainingsService(this.httpClient,this.serverUrl,this.httpService);
...
// context menu
  onContextMenu(event: MouseEvent, item: Trainings) {
...
export class ExampleDataSource extends DataSource<Trainings> {
  ...
  renderedData: Trainings[] = [];
  constructor(
    public exampleDatabase: TrainingsService,
  ...
  sortData(data: Trainings[]): Trainings[] {

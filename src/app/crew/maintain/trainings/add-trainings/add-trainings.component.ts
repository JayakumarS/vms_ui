...
@Component({
  ...
  styleUrls: ['./add-trainings.component.sass']
})
export class AddTrainingsComponent implements OnInit {
  ...
  constructor(private fb: FormBuilder,
   ...
   ngOnInit() {
    ...
    get rowDtls() {
      return this.docForm.get('trainingsBeanDtls') as FormArray;
    }
   ...
   addRow(){
    let trainingsBeanDtlsDtlArray=this.docForm.controls.trainingsBeanDtls as FormArray;
    let arraylen=trainingsBeanDtlsDtlArray.length;
    var len = this.docForm.controls["trainingsBeanDtls"].value.length;
  ...
  removeRow(){
    let count = 0;
    const deleteRow = this.docForm.controls.trainingsBeanDtls as FormArray;
    let i = 0;
  ...
  save(){
    if(this.docForm.valid){
      this.trainingsService.saveTrainings(this.docForm.value, this.router, this.notificationService);
    }else{
      this.matError.markFormGroupTouched(this.docForm);
      this.notificationService.showNotification(
        "snackbar-danger",

...
export class AddPayTypesComponent implements OnInit {
  ...
  update() {
    const dtlArray = this.docForm.get('payTypesBeanDtls') as FormArray;
    dtlArray.controls.forEach(control => {
      control.get('code').enable();
    });
    if(this.docForm.valid){
      this.PayTypesService.updatePayTypes(this.docForm.value, this.router, this.notificationService);
    }else{
  ...
  save(){
    if(this.docForm.valid){
      this.PayTypesService.savePayTypes(this.docForm.value, this.router, this.notificationService);
    }else{
      this.matError.markFormGroupTouched(this.docForm);
      this.notificationService.showNotification(
        "snackbar-danger",

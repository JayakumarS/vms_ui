...
@Component({
  ...
  styleUrls: ['./supply-dashboard.component.sass']
})
export class SupplyDashboardComponent implements OnInit {
  ...
  constructor(private fb: FormBuilder,
    ...
    public SupplyDashboardService: SupplyDashboardService,
...
this.filteritemsuperintendentlist();
   ...
   onDateChange(event: any, inputFlag: any, index: number) {
    if(event.target.value!=null){
      let cdate = this.SupplyDashboardService.getDate(event.target.value);
  ...
  onDateInput(inputValue: any, inputFlag: any, index: number) {
    ...
    if(inputValue!=""){
      if (dateFormat.test(inputValue)) {
        let fdate = this.SupplyDashboardService.getDateObj(inputValue);
        if (inputFlag == "fromdate") {
  ...
  onSubmit() {
    ...
    const data = [26, 47, 45, 0, 16, 9];
    
    // Labels for the bar graph
    const labels = ['Requisitions', 'Quotations', 'Orders', 'Warehouse Deliveries', 'Vessel Deliveries', 'Invoices (Not Finalized)'];
    
    // Get the context of the canvas element we want to select
    const ctx = document.getElementById('myChart') as HTMLCanvasElement;

    // Set the height of the canvas element
    ctx.height = 80; // Change this value to adjust the height as needed

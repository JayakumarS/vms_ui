...
@Component({
  selector: 'app-add-freight-type',
  templateUrl: './add-freight-type.component.html',
  styleUrls: ['./add-freight-type.component.sass']
})
export class AddFreightTypeComponent implements OnInit {
  ...
  fetchDetails(id){
    this.httpService.get<any>(this.FreightTypeService.editUrl+"?id="+id).subscribe({next: (data: any) => {
      this.docForm.patchValue({
        ...
        'freighttypeid': data.list[0].freighttypeid
  ...
  update() {
    ...
    if(this.docForm.valid){
      this.FreightTypeService.updateFreightType(this.docForm.value, this.router, this.notificationService);
    }else{
  ...
  save(){
    if(this.docForm.valid){
      this.FreightTypeService.saveFreightType(this.docForm.value, this.router, this.notificationService);
    }else{
      this.matError.markFormGroupTouched(this.docForm);
      this.notificationService.showNotification(
        "snackbar-danger",
